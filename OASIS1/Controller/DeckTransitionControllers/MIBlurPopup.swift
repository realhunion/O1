...
open class MIBlurPopup: NSObject {
    ...
    private func animatePresent(transitionContext: UIViewControllerContextTransitioning) {
        ...
        // Conform to View Specififications of parentView
        if let parentView = presentedViewController.presentingViewController?.view {
            let maskLayer = CAShapeLayer()
            let maskPath: UIBezierPath = UIBezierPath(roundedRect: parentView.bounds, byRoundingCorners: ([.topLeft, .topRight]), cornerRadii: CGSize(width: 10.0, height: 10.0))
            maskLayer.frame = parentView.bounds
            maskLayer.path = maskPath.cgPath
            visualEffectBlurView.layer.mask = maskLayer
        }
